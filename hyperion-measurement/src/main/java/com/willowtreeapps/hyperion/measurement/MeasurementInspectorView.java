...
class MeasurementInspectorView extends ContentView {
    ...
    MeasurementInspectorView(Context context) {
        super(context);
        HyperionCore.<MeasurementInspectorComponent>getComponent(context).inject(this);

        paintDashed = new Paint();
        paintDashed.setColor(ContextCompat.getColor(context, R.color.selection_primary));
        ...
        paintPrimary.setColor(ContextCompat.getColor(context, R.color.selection_primary));
        paintPrimary.setStyle(Paint.Style.STROKE);
        paintPrimary.setStrokeWidth(6f);

        paintSecondary = new Paint();
        paintSecondary.setColor(ContextCompat.getColor(context, R.color.selection_secondary));
        paintSecondary.setStyle(Paint.Style.STROKE);
        paintSecondary.setStrokeWidth(6f);

        paintText = new TextPaint();
        paintText.setColor(ContextCompat.getColor(context, R.color.selection_primary));
        paintText.setTextSize(45);
        paintText.setStyle(Paint.Style.FILL_AND_STROKE);
        paintText.setStrokeWidth(2);

        measurementTextOffset = getResources().getDimensionPixelSize(R.dimen.measurement_text_offset);

        View current = target.get();
        if (current != null) {
            rectPrimary = new Rect();
            measurements.getScreenLocation(this, current, rectPrimary);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (root == touchTarget || target.get() == touchTarget) {
                target.set(null);
                rectPrimary = null;
                rectSecondary = null;
            } else if (rectPrimary == null) {
                target.set(touchTarget);
                rectPrimary = new Rect();
                measurements.getScreenLocation(this, touchTarget, rectPrimary);

                setWidthMeasurementText(rectPrimary.width());
                setHeightMeasurementText(rectPrimary.height());
            } else {
                rectSecondary = new Rect();
                measurements.getScreenLocation(this, touchTarget, rectSecondary);
    ...
    private View findTarget(View root, float x, float y) {
        if (root instanceof ViewGroup) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                measurements.getScreenLocation(this, child, outRect);
    ...
    private void setWidthMeasurementText(@Px int measurement) {
        ...
        Spannable widthText = new SpannableString(measurements.toDp(measurement) + "dp");
    ...
    private void setHeightMeasurementText(@Px int measurement) {
        ...
        Spannable heightText = new SpannableString(measurements.toDp(measurement) + "dp");
    ...
    private void setLeftMeasurementText(@Px int measurement) {
        ...
        Spannable text = new SpannableString(measurements.toDp(measurement) + "dp");
